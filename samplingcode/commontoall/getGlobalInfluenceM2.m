...
if length(structsV) == 1
   ...
   if ~prod(double(ismember(structsV,structIndV)))
       influenceM = [];
   ...
   for structNum = structsV;
    ...
    influenceM = spalloc(prod(getUniformScanSize(planC{indexS.scan}(getStructureAssociatedScan(structsV(1))))), numPBs, maxnnz);
    numVoxel = prod(getUniformScanSize(planC{indexS.scan}(getStructureAssociatedScan(structsV(1)))));
    ...
    for PBNum = 1 : numPBs
        ...
        for structNum = structsV
            ...
            if ~isempty(beamlets(strBmletInd,PBNum).influence)
                ...
                if pointer>1
                    currIndv = allIndV(currPointer:pointer-1);
                end
                if size(currIndv,1)>0
                    val = intersect(currIndv,indV);
                end
                if size(val,1)>0
                    indicies = find(ismember(allIndV,val));
                    index = indicies(indicies>=currPointer);
                    allDoseV(index) = 0;
                    val=[];
                end
